...
void TaintTracker::Traceker(const llvm::Function* func, std::vector<NodeID> paramNodeIDs) {
    // 初始化，清空
    ...
    for (const auto& inst : targetedinst) {
        ...
        if (llvm::isa<llvm::CallInst>(inst)) {
            NapiHandler::getInstance().dispatch(inst, taintMap, svfg, pag, summaryItems);
        }
    }
    // 处理返回值
    ...
    if (funRet) {
        ...
        std::vector<SVF::NodeID> existingNodeIDs = getTaintmapExistingNodes(funRetNodeIDs, taintMap);
    ...
    SVFUtil::outs() << "\n";
    SummaryExporter::exportToFile(summaryItems, func->getName().str());
}
