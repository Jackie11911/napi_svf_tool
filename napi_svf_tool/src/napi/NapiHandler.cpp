...
void NapiHandler::dispatch(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems) {
    ...
    if (it != handlerMap.end()) {
        it->second(inst, taintMap, svfg, pag, summaryItems);  // 调用注册的处理函数
    }
    return;
}