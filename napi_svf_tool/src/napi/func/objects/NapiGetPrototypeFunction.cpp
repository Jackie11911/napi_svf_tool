...
void handleNapiGetPrototypeFunction(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems) {
    ...
    const SVFVar* objectParamSVFVar = pag->getGNode(objectParamNodeID);
    handleTaintFlow(svfg, pag, objectParamNodeID, objectParamSVFVar, taintMap, summaryItems, summaryItemResult);
...
namespace {
    struct NapiGetPrototypeRegister {
        NapiGetPrototypeRegister() {
            NapiHandler::getInstance().registerHandler("napi_get_prototype", handleNapiGetPrototypeFunction);
        }
    };

    static NapiGetPrototypeRegister _napi_get_prototype_reg;
}