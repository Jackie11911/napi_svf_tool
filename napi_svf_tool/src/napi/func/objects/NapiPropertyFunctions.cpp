...
void handleNapiPropertyFunction(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems) {
    ...
    if(calledFunctionName == "napi_set_property" || calledFunctionName == "napi_set_named_property"){
        for(int i = 2; i < callInst->getNumOperands(); i++){
