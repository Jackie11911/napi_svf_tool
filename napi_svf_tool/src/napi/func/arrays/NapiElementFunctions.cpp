...
void handleNapiElementFunction(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems) {
    ...
    const SVFVar* objectSVFVar = pag->getGNode(objectParamNodeID);
    handleTaintFlow(svfg, pag, objectParamNodeID, objectSVFVar, taintMap, summaryItems, summaryItemResult);
    ...
    if (llvm::isa<llvm::Constant>(indexParam)){
        ...
        NodeID indexParamNodeID = LLVMModuleSet::getLLVMModuleSet()->getValueNode(indexParam);
        const SVFVar* indexSVFVar = pag->getGNode(indexParamNodeID);
        handleTaintFlow(svfg, pag, indexParamNodeID, indexSVFVar, taintMap, summaryItems, summaryItemResult);
    }
    ...
    if(calledFunctionName == "napi_set_element"){
        handleTaintFlow(svfg, pag, resultParamNodeID, resultSVFVar, taintMap, summaryItems, summaryItemResult);
    }
    else{
