...
void handleNapiDataviewFunction(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems) {
    ...
    if(calledFunctionName == "napi_create_dataview") {
        ...
        const SVFVar* arraybufferSVFVar = pag->getGNode(arraybufferParamNodeID);
        handleTaintFlow(svfg, pag, arraybufferParamNodeID, arraybufferSVFVar, taintMap, summaryItems, summaryItemResult);
        ...
        if(llvm::isa<llvm::Constant>(byteOffsetParam)){
            ...
            NodeID byteOffsetParamNodeID = LLVMModuleSet::getLLVMModuleSet()->getValueNode(byteOffsetParam);
            const SVFVar* byteOffsetSVFVar = pag->getGNode(byteOffsetParamNodeID);
            handleTaintFlow(svfg, pag, byteOffsetParamNodeID, byteOffsetSVFVar, taintMap, summaryItems, summaryItemResult);
        }
        ...
        const SVFVar* dataviewSVFVar = pag->getGNode(dataviewParamNodeID);
        handleTaintFlow(svfg, pag, dataviewParamNodeID, dataviewSVFVar, taintMap, summaryItems, summaryItemResult);
