...
void handleNapiGetCbInfo(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems) {
    ...
    int cbInfoID = taintMap.assignNewId(cbInfoParamNodeID);
    ...
    if(argcNodeID != 0){
        ...
        if(argcValue == -1){
            ...
            summaryItemResult.addOperand("int "+std::to_string(argcValue));
        }
    }
    for(int i = 0;i < 3;i++){
        summaryItemResult.addOperand("top");
