...
class NapiHandler {
public:
    using HandlerFunc = std::function<void(const llvm::Instruction*, TaintMap&, const SVFG*, const SVFIR*, std::vector<SummaryItem>&)>;

    static NapiHandler& getInstance();

    void dispatch(const llvm::Instruction* inst, TaintMap& taintMap, const SVFG* svfg, const SVFIR* pag, std::vector<SummaryItem>& summaryItems);
