...
public:
    ...
    static std::map<std::string, llvm::Function*> analyzeGlobalVars(std::set<llvm::GlobalVariable*> globalVars, ReadArkts& readArkts);
    // 处理通过napi_set_named_property注册的函数
    static std::map<std::string, llvm::Function*> analyzeNamedProperties(SVF::SVFG* svfg, SVF::SVFIR* pag, SVF::PointerAnalysis* pta, ReadArkts& readArkts);
